...
import { FieldInput } from "../SectionField";
import { isValidEmail } from "../../utils/email";
import DropdownSelect from "../DropdownSelect";
import { useSnackbar } from "../../context/SnackbarContext";
import { EconomicPlan, Organization, Person, Salon, Template } from "../../types";
import {
  ParticipantDraft,
  listenPlans,
  listenSalons,
...
import { isR2Configured, uploadParticipantPhoto } from "../../data/r2Repository";
...
function draftFromPerson(person: Person): ParticipantDraft {
  return {
    name: person.name,
    templateId: person.templateId,
    status: person.status,
    baseData: person.baseData,
    planId: person.planId ?? "",
    salonIds: person.salonIds ?? [],
...
// Formulario de alta/edición de un participante, aislado de la lista para no saturar una sola pantalla.
export default function ParticipantForm({ participant, onSaved }: Readonly<ParticipantFormProps>) {
  const { membership } = useAuth();
  const { showSnackbar } = useSnackbar();
  const editingId = participant?.id ?? null;
  ...
  const [draft, setDraft] = useState<ParticipantDraft>(participant ? draftFromPerson(participant) : emptyDraft);
      ...
      {!selectedPlan ? (
        <>
          <TextInput
            ...
            style={styles.input}
          />
          {planSearch.trim() && planSearchResults.length > 0 ? (
