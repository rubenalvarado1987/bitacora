...
import { ProfileRecord, Salon } from "../../types";
import {
  ProfileDraft,
  listenSalons,
  saveProfile,
...
import { provisionLinkedAccount } from "../../data/accountProvisioning";
import { updateLinkedAccountCredentials } from "../../data/accountManagement";
import { formatRut, isValidRut } from "../../utils/rut";
import { isValidEmail } from "../../utils/email";
import { isR2Configured, uploadProfilePhoto } from "../../data/r2Repository";
...
function draftFromProfile(profile: ProfileRecord): ProfileDraft {
  return {
...
// Formulario de alta/edición de un perfil (profesional), aislado de la lista.
export default function ProfileForm({ profile, onSaved }: Readonly<ProfileFormProps>) {
  ...
  const [draft, setDraft] = useState<ProfileDraft>(profile ? draftFromProfile(profile) : emptyDraft);
  ...
  const title = useMemo(() => (editingId ? "Editar profesional" : "Nuevo profesional"), [editingId]);
  ...
  const handleUpdateAccount = async () => {
    if (!membership?.organizationId || !draft.linkedUid || !editingId) return;
  ...
  const handleSave = async () => {
    ...
    try {
      await saveProfile(
        membership.organizationId,
        { ...draft, linkedUid, accountEmail: draft.accountEmail || email.trim() || undefined },
        editingId ?? undefined
      );
  ...
  return (
    <View style={styles.card}>
      ...
      <TextInput
        ...
        style={styles.input}
      />
      {salonSearch.trim() && salonSearchResults.length > 0 ? (
      ...
      <Pressable onPress={handleSave} disabled={saving} style={[styles.primaryButton, saving && styles.primaryButtonDisabled]}>
        {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryButtonText}>Guardar profesional</Text>}
      </Pressable>
    </View>
  );
}
